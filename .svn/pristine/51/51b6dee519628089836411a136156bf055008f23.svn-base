 ...
 * JqueryHelper
 *
 * JqueryHelper is a cakePHP helper for auto compressing Javascript codes and placing them into 1 generated file
 * for quality,faster and optimized page loading.
 * You will see minified/compressed into 1 file in js/jscache
 ...
 */
	...
	var $__ext = ".js";
	var $__files = array();
    var $__cacheDir = "jscache";
	var $__scriptBlock = "";
	var $__rewrite = false;
    var $__options = array();    
    var $__includedScript = array();
/**
 ...
 */
	...
	function readFileContent($pathAndFile) {
		$contents = '';
        $pathAndFile = str_replace("/", DS, $pathAndFile);
	...
	function __writeToCacheFile($cacheFile, $content) {
        ...
        cache(str_replace(WWW_ROOT, '', JS . $this->__cacheDir . DS) . $cacheFile, $mergeAll, $this->__cacheTime, 'public');
	}
	...
	function _linkToJsCache($cacheFile) {
		$out = sprintf($this->tags['javascriptlink'], $this->webroot(JS_URL . $this->__cacheDir . "/" . $cacheFile . $this->__ext));
		return $out;
	}
    ...
    private function clearCache() {
        if (isset($this->params->query['clear'])) {
            if($this->params->query['clear'] == "view"){
                clearCache();
            } else {
                Cache::clear();
            }
