<div class="calendarEvents index">
	<h2><?php echo __('Calendar Events'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('calendar_id'); ?></th>
			<th><?php echo $this->Paginator->sort('title'); ?></th>
			<th><?php echo $this->Paginator->sort('description'); ?></th>
			<th><?php echo $this->Paginator->sort('date_start'); ?></th>
			<th><?php echo $this->Paginator->sort('date_end'); ?></th>
			<th><?php echo $this->Paginator->sort('timezone'); ?></th>
			<th><?php echo $this->Paginator->sort('is_all_day'); ?></th>
			<th><?php echo $this->Paginator->sort('is_repeat'); ?></th>
			<th><?php echo $this->Paginator->sort('location'); ?></th>
			<th><?php echo $this->Paginator->sort('color'); ?></th>
			<th><?php echo $this->Paginator->sort('availability'); ?></th>
			<th><?php echo $this->Paginator->sort('privacy'); ?></th>
			<th><?php echo $this->Paginator->sort('created'); ?></th>
			<th><?php echo $this->Paginator->sort('modified'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	<?php
	foreach ($calendarEvents as $calendarEvent): ?>
	<tr>
		<td><?php echo h($calendarEvent['CalendarEvent']['id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($calendarEvent['Calendar']['name'], array('controller' => 'calendars', 'action' => 'view', $calendarEvent['Calendar']['id'])); ?>
		</td>
		<td><?php echo h($calendarEvent['CalendarEvent']['title']); ?>&nbsp;</td>
		<td><?php echo h($calendarEvent['CalendarEvent']['description']); ?>&nbsp;</td>
		<td><?php echo h($calendarEvent['CalendarEvent']['date_start']); ?>&nbsp;</td>
		<td><?php echo h($calendarEvent['CalendarEvent']['date_end']); ?>&nbsp;</td>
		<td><?php echo h($calendarEvent['CalendarEvent']['timezone']); ?>&nbsp;</td>
		<td><?php echo h($calendarEvent['CalendarEvent']['is_all_day']); ?>&nbsp;</td>
		<td><?php echo h($calendarEvent['CalendarEvent']['is_repeat']); ?>&nbsp;</td>
		<td><?php echo h($calendarEvent['CalendarEvent']['location']); ?>&nbsp;</td>
		<td><?php echo h($calendarEvent['CalendarEvent']['color']); ?>&nbsp;</td>
		<td><?php echo h($calendarEvent['CalendarEvent']['availability']); ?>&nbsp;</td>
		<td><?php echo h($calendarEvent['CalendarEvent']['privacy']); ?>&nbsp;</td>
		<td><?php echo h($calendarEvent['CalendarEvent']['created']); ?>&nbsp;</td>
		<td><?php echo h($calendarEvent['CalendarEvent']['modified']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $calendarEvent['CalendarEvent']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $calendarEvent['CalendarEvent']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $calendarEvent['CalendarEvent']['id']), null, __('Are you sure you want to delete # %s?', $calendarEvent['CalendarEvent']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>

	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Calendar Event'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Calendars'), array('controller' => 'calendars', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Calendar'), array('controller' => 'calendars', 'action' => 'add')); ?> </li>
	</ul>
</div>
